...
$(document).ready(function(e) {
	...
	*/
	...
	$('input[type="submit"][name="ajaxBut"], input.ajaxBut[type="button"]').click(function(e) {
		e.preventDefault();
		$.fn.ajax_validate($(this).parents('form'));
	});
	...
	$.fn.ajax_validate = function(e) {
		...
		if(e.hasClass("UI_FORM_AJAX_SAVE")) {
			...
			error_identifier = '';
			////////////////////////////// Validate Required Input[type="text"] fields /////////////////////////
			$('form#'+currFormId+' input[type="text"], form#'+currFormId+' input[type="number"], form#'+currFormId+' input[type="email"]').each(function(index, element) {
				if($(this).hasClass('required')) {
					if($(this).val()=='') {
						$(this).addClass("errorclass");
						errorStr += $.fn.getTitle($(this))+" Cannot be blank<br />";
			...
			if(hasError==true) {
				...
				callBackFnToSaveData = $('form#'+currFormId+' input[name="callBackFnToSaveData"]').val();
				var formObj = $('form#'+currFormId).serializeObject();
				if($('form#'+currFormId).find("span.loading-icon").length>0) {
					$('form#'+currFormId).find("span.loading-icon").css("display","inline-block");
				}
				eval(callBackFnToSaveData+'('+JSON.stringify(formObj)+');');
